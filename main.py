...
def open_rms(parent_window=None):
    ...
    """
    rms_window = Toplevel(parent_window) if parent_window else CTk()
    ...
    try:
        ...
        logo_label = CTkLabel(rms_window, image=logo, text='')
        logo_label.image = logo  # Prevent garbage collection
        logo_label.pack(pady=20)
    except Exception as e:
        Label(rms_window, text=f"Error loading logo: {e}", font=("Arial", 14)).pack(pady=20)

    Label(rms_window, text="Welcome to the RMS System", font=("Arial", 16)).pack(pady=20)
...
def extract_skills(resume_text):
    ...
    data = pd.read_csv(r'C:\Users\HP\OneDrive\Desktop\Resume details\PARSIGHT-Resume-Parser-\csv\skills.csv')
...
def upload_document():
    ...
    try:
        ...
        try:
            skills_data = pd.read_csv(r'C:\Users\HP\OneDrive\Desktop\Resume details\PARSIGHT-Resume-Parser-\csv\ParSight_Dataset_merged(Updated).csv')
...
def candidate_selection():
    ...
    Uses a condition to load and display the RMS system UI.
    """
    condition = True
    if condition:
        try:
            subprocess.Popen(["python","rms.py"])
        except ImportError:
            messagebox.showerror("Error", "RMS module not found. Ensure rms.py is in the correct location.")
        except Exception as e:
...
image_resized = image.resize((1590, 300))
...
upload_button_image = PhotoImage(file=r"C:\Users\HP\OneDrive\Desktop\Resume details\PARSIGHT-Resume-Parser-\gui\build\assets\frame0\button_1.png")
